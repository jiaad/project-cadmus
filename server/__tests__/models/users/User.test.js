...
describe('Should not create Users', () => {
  it('sshould send error msg (name.first) and not create an User', async () => {
    expect.assertions(3)
    params.name.first = ''
    try {
      const newUser = new User(params)
      const unsavedUser = await newUser.save()
    } catch (e) {
      console.log(typeof e.message, err)
      expect(e).toBeTruthy()
  ...
  it('should send error msg (isActive) and not create an User ', async () => {
    ...
    const param = params.isActive = null
